    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedClasses?.map((cls) => {
          //we are adding current day to the class date to make comparison
          ...
          return (
            ...
            <div key={cls.id}  className={`rounded-lg border shadow-sm p-6 h-full ${isCurrent ? 'bg-green-50 border-green-200' :
                isPast ? 'bg-gray-50 border-gray-200' :
                  'bg-white border-blue-200 hover:border-blue-400'
              }`}>
              <div className="flex items-center justify-between mb-4">
                <span className={`px-3 py-1 rounded-full text-sm ${isCurrent ? 'bg-green-100 text-green-800' :
